...
```{r}
#Calculates TBI using matrices from first time period and second time period
...
#creates table with TBI results (also see bcplot file)
TBI <- as.data.frame(z[[1]])
p.TBI <- as.data.frame(z[[2]])
BCD <- as.data.frame(z[[4]])

#binds to station names for TBI results 
TBI.results <- cbind(Stations, TBI, p.TBI, BCD)

#merge with the standardized data to create dataframe for LME modelling 
TBI.predictors <- merge(TBI.results, trca_standardized, by = "Station")
```
##Plotting BC Plot based on TBI
```{r}
#this plots the BC plot data with design features (added in watershed separately)
bcplot %>% 
...
#creating dataset with subset estimates to prep for ggplot plotting
...
#renaming the rows to look better
rownames(beta) <- c("(Intercept)", "Woodland T1","Woodland loss %", "Wetland T1", "Weltand loss %", "Urban T1", "Urban Intensif. %")
...
e<- as.data.frame(e)
rownames(e) <- c("(Intercept)", "Wetland T1","Wetland loss %", "Woodland T1", "Woodland loss %", "Urban T1", "Urban Intensif. %")
...
k <- as.data.frame(k)
#renaming rows
rownames(k) <- c("(Intercept)", "DCI T1","DCI Δ","Wetland T1", "Wetland loss %", "Urban T1", "Urban Intensif. %", "Woodland T1", "Woodland loss %")
#making column for variables names
k <- cbind(Variable = rownames(k), k)
#renaming columns
colnames(k) <- c("Variable", "Full", "Subset", "lower.con", "upper.con")
shan.div.model.avg <- k 
```
####Model avg estimate plot
```{r}
#plotting estimates!
shan.averaged.plot <- shan.div.model.avg  %>% 
  ggplot(aes(x = Subset, y = Variable)) +
  geom_point(size = 2.5) +
  theme_bw() +
...
##Full dataset
```{r}
t <- tpaired.krandtest(pivot_first_m, pivot_last_m, nperm = 999, list.all = FALSE)
summary(z)
...
#not enough observations to run this since only one observation
...
```{r}
#analzing only with p/a data, difference is in the pa.tr part of code
...
TBI.results.pa <- cbind(Stations, TBI.pa, p.TBI.pa, BCD.pa)

colnames(TBI.results.pa) <- c("Station", "TBI", "p.TBI", "B/(2A+B+C)", "C/(2A+B+C)", "D=(B+C)/(2A+B+B)", "Change" )
